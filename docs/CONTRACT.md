...
- model: `Installation{id,external_id,name,excluded_models:Vec<String>,
  ...
  `AccessPolicy{allowed_models,denied_models,allowed_providers,denied_providers:Vec<String>}`
  with `is_unrestricted()`, `permits_model(&str)`, `permits_provider(&str)`.
...
- routing: `WireFormat{Anthropic,OpenaiChat,OpenaiResponses,Gemini}` (`as_str`),
  ...
  `Deployment{model_name,provider:String,kind:ProviderKind,upstream_model,
  ...
  excluded_models:BTreeSet<String>,enabled_providers:Option<BTreeSet<String>>,
